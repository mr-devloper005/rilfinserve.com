...
const taskDeck: Record<TaskKey, { icon: typeof FileText; archiveClass: string; promise: string; badge: string }> = {
  ...
  listing: { icon: Building2, archiveClass: 'grid gap-5 md:grid-cols-2', promise: 'Directory cards highlight company identity, service category, location, contacts, and quick action details.', badge: 'Business' },
...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const archiveVars = { '--archive-bg': '#fbfcff', '--archive-text': '#11142f', '--archive-surface': '#ffffff', '--archive-accent': '#3f6ff2', '--editable-border': 'rgba(17,20,47,0.10)', '--editable-container': '1180px' } as CSSProperties
        ...
        <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_360px] lg:px-8 lg:py-16">
          <div className="rounded-md border border-[var(--editable-border)] bg-[var(--archive-surface)] p-7 shadow-[0_18px_48px_rgba(17,20,47,0.06)] sm:p-9">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={basePath} className="rounded-md bg-[#3f6ff2] px-5 py-3 text-sm font-black text-white">Browse all</Link>
              <Link href="/search" className="rounded-md border border-[var(--editable-border)] bg-white px-5 py-3 text-sm font-black">Search services</Link>
            </div>
          </div>

          <form action={basePath} className="self-end rounded-md border border-[var(--editable-border)] bg-white p-5 shadow-sm">
            ...
            <button className="mt-3 h-12 w-full rounded-md bg-[#3f6ff2] text-sm font-black text-white">Apply</button>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-8">
          {posts.length ? (
            ...
            <div className="rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-white/60 p-10 text-center">
              ...
              <h2 className="mt-4 text-3xl font-black tracking-[-0.05em]">No listings found</h2>
              <p className="mt-2 text-sm opacity-65">Try another category or refresh this page after publishing new business content.</p>
            </div>
          )}
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            ...
            <span className="rounded-full bg-[#11142f] px-5 py-3 text-sm font-black text-white">Page {page} of {pagination.totalPages || 1}</span>
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group grid gap-5 rounded-md border border-[var(--editable-border)] bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(17,20,47,0.10)] sm:grid-cols-[112px_1fr]">
      <div className="flex h-28 w-28 items-center justify-center overflow-hidden rounded-full bg-[var(--archive-bg)] ring-1 ring-[var(--editable-border)]">
        {logo ? <img src={logo} alt="" className="h-full w-full object-cover" /> : <BriefcaseBusiness className="h-10 w-10 opacity-45" />}
      </div>
      <div className="min-w-0">
        <div className="flex flex-wrap gap-2">
          <span className="rounded-full bg-[#3f6ff2] px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-white">Business</span>
